...
const float SynthController::range[] = {
    ...
    // Em pentatonic scale - middle octave (top limit)
    N_E4, N_G4, N_A4, N_B4,          // Stop at B4 - no D5
...
void SynthController::createBowlPattern(uint8_t numSteps, uint16_t bpm, uint16_t seedValue) {
    ...
    for (uint8_t i = 0; i < numSteps; i++) {
        ...
        bool active = (i % 4 == 0) || (i % 7 == 0) || (random(100) < 40);
        
        if (active) {
            float note = range[random(numBowlFreqs)];
            uint8_t velocity = random(60, 90);      // Gentle dynamics
